(function($, Dnxte_Essential) {
  ...
  const adminPagadata = ["modules", "extensions"];
  adminPagadata.forEach(function(item) {
    const $adminForm = $(`#dnxte-admin-${item}-form`),
        ...
        success: function(res) {
          ...
          if (res.success) {
            var t = setTimeout(function() {
              $saveButton.attr("disabled", true).text("Changes Saved");
              clearTimeout(t);
            }, 500);
          }
        },
    ...
    $adminForm.find(".dnxte-toggle-all").click(function(e) {
      if ($adminForm.find(".dnxte-toggle-all-wrap input").is(":checked")) {
        console.log($(this).is(":checked"));
        $adminForm
          .find(".dnxte-toggle-check:visible")
          .prop("checked", false)
          .change();
        $adminForm.find(".dnxte-toggle-all").text("Disable All");
      } else {
        $adminForm
          .find(".dnxte-toggle-check:visible")
          .prop("checked", true)
          .change();
        $adminForm.find(".dnxte-toggle-all").text("Enable All");
      }
    });
  });
  // Search Filter
