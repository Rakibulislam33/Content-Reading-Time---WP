...
// If you use without dynamic content then use it.
const HeadTitle = ({
  title,
  tag: HeadingTag = "h2",
  classes = "",
  ...
  return <HeadingTag className={classes}>{title}</HeadingTag>;
};
...
const ButtonText = ({ utils, iconSlug, props, text, link, linkTarget, classes = "dnxte-button", hasWrapper = false, defaultIcon }) => {
  ...
  let frontIcon = oldFrontIcon[0] ? utils.processFontIcon(oldFrontIcon[0]) : utils.processFontIcon(defaultIcon);
  ...
  if ( hasWrapper ) {
    return (
    <div className = "dnxte_button_wrapper">
      <a   className = {classes} data-icon = {frontIcon} data-icon-tablet = {frontIconTablet} data-icon-phone = {frontIconPhone} href = {validLink} target = {linkTarget}>
        {text}
      </a>
    </div>
    )
...
const applyBgCss = (css, props, backgroundObject) => {
  ...
  const gradient_repeat =
    props[slug + "_gradient_repeat"] &&
    props[slug + "_gradient_repeat"] === "on"
      ? "repeating-"
      : "";
  ...
  if (use_color_gradient === "on") {
    const gradientTypeObj = {
      linear: "linear",
      conic: "conic",
      circular: "radial",
      elliptical: "radial",
    };
    const gradientTypePosition = {
      linear: gradient_direction,
      circular: `circle at ${gradient_radial}`,
      elliptical: `ellipse at ${gradient_radial}`,
      conic: `from ${gradient_direction} at ${gradient_radial}`,
    };

    const direction = gradientTypePosition[gradient_type];
    gradient_bg = `${gradient_repeat}${gradientTypeObj[gradient_type]}-gradient(${direction}, ${gradient_stops})`;
    gradient_bg_tablet = `${gradient_repeat}${gradientTypeObj[gradient_type]}-gradient(${direction}, ${gradient_stops_tablet})`;
    gradient_bg_phone = `${gradient_repeat}${gradientTypeObj[gradient_type]}-gradient(${direction}, ${gradient_stops_phone})`;
    gradient_bg_hover = is_hover_enabled
      ? `${gradient_repeat}${gradientTypeObj[gradient_type]}-gradient(${direction}, ${gradient_stops_hover})`
      : "";
...
export {
  Button, ButtonText, Description, HeadTitle, Icon, IconUsingPsuedo, Image, SliderArrow, Title, applyAllBgCss, applyBgCss, applyCss,
  applyCustomSpacing, applyPsuedoIconCss, getAlignment,
  getBlurbAlignment, getShapes, imageFunc, paginationClass, renderStars
};
